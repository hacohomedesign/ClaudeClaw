...
async function main() {
  ...
  // ── 6. Config folder + CLAUDE.md personalization ─────────────────────────
  section('Personal config folder');

  info('Your assistant configuration (CLAUDE.md) lives outside the repo.');
  info('This keeps your name, vault paths, and AI persona out of version control.');
  console.log();

  const defaultConfigDir = '~/.claudeclaw';
  const reservedDir = path.join(os.homedir(), '.claude');

  let rawConfigDir: string;
  let configDir: string;

  while (true) {
    rawConfigDir = await ask('Config folder path', defaultConfigDir);
    configDir = rawConfigDir.startsWith('~/')
      ? path.join(os.homedir(), rawConfigDir.slice(2))
      : path.resolve(rawConfigDir);

    if (configDir === reservedDir) {
      warn('~/.claude is reserved by Claude Code and cannot be used as the config folder.');
      continue;
    }

    // If the folder already exists, confirm before using it
    if (fs.existsSync(configDir)) {
      warn(`${configDir} already exists.`);
      const useExisting = await confirm('Use this existing folder?', true);
  ...
  if (!fs.existsSync(workspaceDir)) {
    ...
    ok(`Created workspace: ${workspaceDir}`);
  } else {
    ok(`Using existing workspace: ${workspaceDir}`);
  }

  const claudeDestPath = path.join(workspaceDir, 'CLAUDE.md');
  const claudeExamplePath = path.join(PROJECT_ROOT, 'CLAUDE.md.example');

  if (fs.existsSync(claudeDestPath)) {
    warn(`CLAUDE.md already exists at ${claudeDestPath}`);
    const overwrite = await confirm('Overwrite with the example template?', false);
    if (overwrite && fs.existsSync(claudeExamplePath)) {
      fs.copyFileSync(claudeExamplePath, claudeDestPath);
      ok('Replaced with CLAUDE.md.example');
    } else {
      info('Keeping existing CLAUDE.md.');
    }
  } else if (fs.existsSync(claudeExamplePath)) {
    fs.copyFileSync(claudeExamplePath, claudeDestPath);
    ok(`Copied CLAUDE.md.example → ${claudeDestPath}`);
  }

  console.log();
  info('At minimum, replace the [BRACKETED] placeholders in CLAUDE.md:');
  ...
  if (openClaude) {
    ...
    try {
      spawnSync(editor, [claudeDestPath], { stdio: 'inherit' });
    } catch {
      warn(`Could not open ${editor}. Edit manually: ${claudeDestPath}`);
    }
  }
  ...
  const lines = [
    ...
    `CLAUDECLAW_CONFIG=${rawConfigDir}`,
    '',
    '# ── Required ──────────────────────────────────────────────────',
  ...
  const known = new Set(['CLAUDECLAW_CONFIG','TELEGRAM_BOT_TOKEN','ALLOWED_CHAT_ID','ANTHROPIC_API_KEY','GROQ_API_KEY','ELEVENLABS_API_KEY','ELEVENLABS_VOICE_ID','GOOGLE_API_KEY','CLAUDE_CODE_OAUTH_TOKEN','WHATSAPP_ENABLED']);
  ...
  console.log();
  info(`Edit ${workspaceDir}/CLAUDE.md any time to change personality, add context, or update skills.`);
